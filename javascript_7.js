const cats = [
    {
        ...
        adoptionStatus: null, //Initially not adopted
    },
    {
        name: "Quinton",
        breed: "Tabby",
        ...
        adoptionStatus: undefined, //Adoption status undecided
    },
];

function Cat(name, breed, age, imageUrl, description) {
    this.name = name;
    this.breed = breed;
    this.age = age;
    ...
    this.toggleAdoptionStatus = function () {
        this.adoptionStatus = this.adoptionStatus === null ? "Adopted" : null;
    };
}
...
function displayCat(cat) {
    ...
    catItem.innerHTML = `
      ...
      <p>Age: ${cat.age}</p>
      <p>${cat.description}</p>
      <p>Adoption Status: ${cat.adoptionStatus || "Not Yet Decided"}</p>
      <button onclick="this.parentElement.firstChild.textContent = '${cat.name} (Adopted)'">Toggle Adoption</button>
    `;
    document.getElementById("cat-list").appendChild(catItem);
}

const mimi = new Cat("Mimi", "Ragdoll", 2, "...", "...");
const quinton = new Cat("Quinton", "Tabby", 4, "...", "...");
